...
sale_data = pd.read_csv(io.BytesIO(response), header=0, delimiter="|", low_memory=False)
...
temp_df = spark.createDataFrame(sale_data, schema= data_schema)

# Update Sale_Date column with TimeStamp datatype
final_df = temp_df.withColumn('Sale_Date', col('Sale_Date').cast("timestamp"))

# Register the DataFrame as a SQL temporary view
final_df.createOrReplaceTempView("dailySaleData")
